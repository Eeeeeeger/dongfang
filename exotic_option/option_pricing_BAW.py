...
for sigma in [0.2, 0.4]:
    for K in [35,40,45]:
        for T in [30, 90, 180, 360]:
            ...
            lt[1].append(K)
            lt[2].append(T)
            lt[3].append(getValue(_EUROPEAN, _VALUE, _PUT, spot, K, T/365, r, r, sigma))
            lt[4].append(getValue(_AMERICAN, _VALUE, _PUT, spot, K, T/365, r, r, sigma))

lt[5] = [0.0061, 0.1213, 0.3585, 0.7678, 0.845, 1.3848, 1.8575, 2.4407, 5, 5.0191, 5.1736, 5.4961,
         0.2416, 1.0065, 1.8928, 3.1491, 1.7542, 2.9449, 4.0369, 5.4582, 5.2671, 6.1161, 7.072, 8.4089
]
import pandas as pd
df = pd.DataFrame({'K': lt[1], 'sigma':lt[0], 'T':lt[2], 'American': lt[4], 'Wind': lt[5]}).set_index(['K','sigma','T'])
df['Diff'] = (df['American'] - df['Wind']).abs()
df['Ratio'] = df['Diff']/ df['American']
print(df)
print(df['Diff'].mean())
df.to_csv('option.csv')