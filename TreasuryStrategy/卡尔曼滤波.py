...
def Kalman1D(observations, ratio=1000):
    ...
    return pred_state.reshape(-1), data-pred_state.reshape(-1)


df = pd.read_excel('MacroData.xlsx', sheet_name='Money', skiprows=1, index_col=0).iloc[:-1]
data = np.array(df['中国:中债国债到期收益率:10年'])
trend, resid = Kalman1D(data, 500)
plt.figure(figsize=(8, 4))
plt.plot(range(len(data)), data, label='Ori')
plt.plot(range(len(data)), trend, label='Trend')
# plt.plot(range(len(data)), b.reshape(-1), label='b')
plt.legend()
plt.show()
